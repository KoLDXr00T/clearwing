...
class NativeAgentGraph:
    def __init__(
        ...
        *,
        ...
        if enable_knowledge_graph and capabilities.has("knowledge"):
            try:
                self.knowledge_graph = KnowledgeGraph(persist_path="~/.clearwing/knowledge_graph.json")
            except Exception:
                logger.warning("Failed to initialize KnowledgeGraph", exc_info=True)

    def stream(self, input_data: dict[str, Any] | Command, config: dict, stream_mode: str = "values"):
        del stream_mode
    ...
    def _resume(self, thread_id: str, approved: bool):
        ...
        state = self._get_or_create_state(thread_id)
        tool_events, paused = self._run_tool_calls(state, pending.tool_calls, resume_decision=approved)
    ...
    def _run_tool_calls(
        ...
        *,
        ...
        for index, tool_call in enumerate(tool_calls):
            ...
            if self.audit_logger:
                self.audit_logger.log_tool_call(tool_name=tool_name, args=tool_args, result=content[:2000])
    ...
    def _invoke_tool(self, tool: AgentTool, arguments: dict[str, Any], resume_decision: object) -> Any:
